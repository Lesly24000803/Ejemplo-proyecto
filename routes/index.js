...
const authenticateToken = (req, res, next) => {
    ...
    if(!token){
    return res.status(401).json({error: "Access denegado. Token no proporcionado." });
...
router.get('/habits', authenticateToken, async (req, res) => {
    try {
        let userId = req.user && req.user.userId ? req.userId : res.status(500).json({message: 'Error retrieving userId'});
...
const timeDifferenceInDays = (date1, date2) => {
  const diffnceMS = Math.abs(date1 - date2);
  return diffnceMS / (1000 * 60 * 60 * 24);
